...
if (isset($_POST['login'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM receptionist WHERE email = ? AND Password = ?");
    $stmt->bind_param("ss", $email, $Password);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($row = $result->fetch_assoc()) {
        $_SESSION['receptinistLogin'] = true;
        $_SESSION['receptionisId'] = $row['Id'];
        header("Location: Rcontrol.php");
        exit();
    } else {
...
<head>
    ...
    <style>
        body {
            ...
            height: 100vh;
            margin: 0;
            background-color: rgba(134, 234, 205, 0.93);
        ...
        .log-container {
            background-color: rgba(156, 193, 109, 0.93);
            padding: 20px;
            width: 360px;
            border-radius: 10px;
